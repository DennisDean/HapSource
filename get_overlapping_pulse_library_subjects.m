function overlaping_subjects_struct = get_overlapping_pulse_library_subjects ...
              (pulse_library_1, pulse_library_2)
...
get_db1_subject_id_F = @(x)getfield(pulse_library_1{x}, 'subject_id_val');
get_db2_subject_id_F = @(x)getfield(pulse_library_2{x}, 'subject_id_val');
get_db1_subj_descr_txt_F = ...
    @(x)pulse_library_1{x}.subject_id_str;
get_db2_subj_descr_txt_F = ...
    @(x)pulse_library_1{x}.subject_id_str;

% Define database subjects
database_1_subjects = ...
    arrayfun(get_db1_subject_id_F, [1:length(pulse_library_1)]);
database_2_subjects = ...
    arrayfun(get_db2_subject_id_F, [1:length(pulse_library_2)]);
...
for s = 1:1:num_overlaping_subjects 
    temp = get_db1_subj_descr_txt_F(db_1_indexes(s));   
    temp = temp{1};
    joint_subject_descriptor_text{s} =  temp;   
end

% Define return
overlaping_subjects_struct.joint_subject_ids = joint_subject_id;
overlaping_subjects_struct.db_1_indexes = db_1_indexes;    
overlaping_subjects_struct.db_2_indexes = db_2_indexes;
overlaping_subjects_struct.joint_subject_descriptor_text = ...
    joint_subject_descriptor_text;    
overlaping_subjects_struct.num_overlaping_subjects = num_overlaping_subjects;
